...
namespace WebApiConcepts.A02._02.Domain.ORM.Seeds
{
    /// <summary>
    /// 使用种子数据初始化数据库
    /// </summary>
    ...
    {
        private readonly DomainDataDbContext? _dataContext = default;
        ...
        public DataSeed(DomainDataDbContext dataContext, UserManager<ApplicationUser> userManager, RoleManager<ApplicationRole> roleManager)
        {
        ...
        public async Task Initial()
        {
            ...
            await _SetApplicationRolesAndUser();

            await _SetPermissionsForDemoRole();
        }
        ...
        /// </summary>
        /// <returns></returns>
        private async Task _SetApplicationRolesAndUser()
        {
            ...
            if (!_dataContext!.ApplicationRoles!.Any())
            {
            ...
            if (!_dataContext!.ApplicationUsers!.Any())
            {
                await _userManager!.CreateAsync(user, "1234@Abcd");
                await _userManager.AddToRoleAsync(user, role.Name);

                var claim = new Claim("服务区域", "Application");
        ...
        /// </summary>
        /// <returns></returns>
        private async Task _SetPermissionsForDemoRole()
        {
            var role = await _roleManager!.FindByNameAsync("系统管理员组");
            if(!(await _roleManager.GetClaimsAsync(role)).Any())
            {
                // 获取全部的许可
                var permissionCollection = LpfwPermissionCollection.All;
                // 为当前角色组添加许可 Claim
